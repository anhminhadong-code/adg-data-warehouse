...
EXCEL_FILE = r"C:\Projects\ADG\DWH\raw-data\storage_group_category.xlsx"

SCHEMA = [
    "storage_group_name",
    "storage_group_category"
]
...
def upsert_data(df):
    ...
    cur.execute("""
        ...
        FROM dim_storage_group_category
    """)
    ...
    for row in df.itertuples(index=False):
        ...
        key = (row.storage_group_name,)

        values = (
            row.storage_group_name,
            row.storage_group_category
        )
    ...
    insert_sql = """
    INSERT INTO dim_storage_group_category (
        storage_group_name,
        storage_group_category
    )
    VALUES (%s,%s)
    ...
    update_sql = """
    UPDATE dim_storage_group_category
    SET
        storage_group_category = %s
    WHERE
    ...
    for row in update_rows:
        ...
        update_values.append((
            row[1],  # storage_group_category
            row[0],  # storage_group_name
        ))
